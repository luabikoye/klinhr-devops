...
if (isset($_POST['btn_credentials'])) {
  ...
  if ($document == 'choose_option') {
    $error = '<div class="alert alert-danger alert-dismissible mt-2">
    <button type="button" class="close" data-dismiss="alert">&times;</button>
    <strong>Document is required </strong>
    </div>';
    include('upload-credentials.php');
    exit;
  }

  $path = '' . $candidate . '_' . $_FILES['document_other']['name'];
  $file_loc = $_FILES['document_other']['tmp_name'];
  $file_size = $_FILES['document_other']['size'];
  $file_type = $_FILES['document_other']['type'];
  $folder = "outsourcehr-admin/document/";
  ...
  if ($_SESSION['cv']) {
    ...
    if ($result) {
      ...
      <strong>Your document was successfully updloaded </strong>';
      ...
      header('location: reference.php?nav=5&succ=yes2');
      exit;
    } else {

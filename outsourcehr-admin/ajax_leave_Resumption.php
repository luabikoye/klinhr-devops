...
try {
    ...
    $countQuery = "SELECT COUNT(*) as total FROM emp_resumption_form $where";
    $countResult = mysqli_query($db, $countQuery);
    $totalRow = mysqli_fetch_assoc($countResult);
    $totalRecords = $totalRow['total'] ?? 0;

    $query = "SELECT * FROM emp_resumption_form $where ORDER BY $orderBy $orderDir LIMIT $start, $length";

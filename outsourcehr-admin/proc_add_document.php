...
if ($caption == '') {
    $error = "Name of document is required";
...
// }
    ...
    if ($file_ext == 'JPEG' || $file_ext == 'jpeg' || $file_ext == 'JPG' || $file_ext == 'jpg' || $file_ext == 'PNG' || $file_ext == 'png' || $file_ext == 'PDF' || $file_ext == 'pdf') {
...
$query = "insert into emp_download set caption = '$caption', filename = '$path',  date_posted = '$date_posted', access_type = '$access_type', size = '$file_size'  ";
...
if ($result) {
    ...
    activity_log($_SESSION['Klin_admin_email'], "Added a new document with title: $caption");

    $success = "Document has been successfully added";
    include('document-forms.php');
    exit;
} else {
    $error = "Document not added, check if all information were correctly entered";
    include('document-forms.php');
    exit;
}
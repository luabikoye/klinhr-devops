...
if (isset($_POST['btn_filter'])) {
    if ($_POST['appraisal_period']) {
        $clause1 = " and appraisal_period = '" . ($_POST['appraisal_period']) . "'";
    }

    if ($_POST['current_job']) {
        $clause2 = " and current_job = '" . $_POST['current_job'] . "'";
    }
    if (($_POST['client'])) {
        $clause3 = " and appraisal_id = '" . get_val('appraisal', 'id', $_POST['client'], 'id') . "' ";
    }

    $query = "select * from emp_appraisald_self where $account_token and status != 'Completed' $clause1$clause2$clause3  order by id desc ";
} else {

    $query = "select * from emp_appraisald_self where $account_token and status != 'Completed' order by id desc  ";
}
...
<head>
    ...
    <!-- Title -->
    <title>Ongoing Appraisal | KlinHR</title>
            ...
            <a class="btn btn-primary" href="javascript:;" data-bs-toggle="modal" data-bs-target="#newProjectModal">
              <i class="bi-plus me-1"></i> New project
                ...
                <ul class="nav nav-tabs page-header-tabs">
                    <li class="nav-item">
                        Ongoing Appraisal List <span class="badge bg-soft-dark text-dark ms-1"><?php echo number_format($num) ?></span>
                    </li>
                    <!-- <li class="nav-item">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="table-column-ps-0">
                                        <span class="d-flex align-items-center">
                                            ...
                                            <div class="flex-grow-1 ms-3">
                                                ...
                                                <span class="d-block fs-6 text-body"><?php echo $row['email']; ?></span>
                                            </div>
                                        </span>
                                    </td>
                                    ...
                                    <td><?php echo $row['appraisal_period']; ?></td>
                                    <td><?php echo $row['status']; ?></i></td>
                                    <td>
                                        <a target="blank" data-toggle="tooltip" data-placement="top" data-original-title="View Applicant" class="btn btn-sm btn-outline-secondary" href="view-onboarding?unique=<?php echo base64_encode($row['email']); ?>"><i class="bi-eye"></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>

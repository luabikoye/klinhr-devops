...
// the admin clicks to move user back to a different stage on the hrbp.php page
if ($_POST['btn_move']) {
    $stage = $_POST['stage'];
    $id = $_POST['id'];

    for ($i = 0; $i < count($id); $i++) {
        $query = "update emp_staff_details set staff_id = '', EmployeeID = '', staff_serial_no = 0, completed = '$stage' where id = '" . $id[$i] . "'";
        mysqli_query($db, $query);

        activity_log($_SESSION['Klin_admin_user'], 'Moved staff with ID ' . $id[$i] . ' to back to ' . $stage);
    }

    $success = base64_encode('Employee moved to ' . $stage);
    header("Location: hrbp?cat=Y2xpZW50&success=$success");
    exit;
}
...
if ($query->num_rows > 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        $lineData = array(
            $row['id'], $row['EmployeeID'], $row['onboarding_code'], $row['staff_id'], $row['candidate_id'],
